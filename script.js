...
async function deleteEmployee(username) {
  ...
  // שלב א: קרא את השם לפני המחיקה
  const snapBefore = JSON.parse(localStorage.getItem(DB_KEY) || '{}');
  const name = (snapBefore.users || {})[username]?.fullName || username;

  if (!confirm(`למחוק את העובד ${name}? הפעולה בלתי הפיכה.`)) return;

  // שלב ב: עצור listener מיד — לפני כל כתיבה
  if (_fbUnsubscribe) { _fbUnsubscribe(); _fbUnsubscribe = null; }
  _isSyncing = true;
  _lastPushTime = Date.now();

  // שלב ג: בנה עותק נקי ללא העובד — ישירות מ-localStorage
  const cleanDB = JSON.parse(localStorage.getItem(DB_KEY) || '{}');
  if (!cleanDB.users) cleanDB.users = {};
  delete cleanDB.users[username];
  if (cleanDB.vacations)        delete cleanDB.vacations[username];
  if (cleanDB.timeClockRecords) delete cleanDB.timeClockRecords[username];
  if (cleanDB.timeclock)        delete cleanDB.timeclock[username];
  if (cleanDB.sick) {
    Object.keys(cleanDB.sick).forEach(k => {
      if (k.startsWith(username + '_')) delete cleanDB.sick[k];
    });
  }
  // הוסף audit entry
  if (!cleanDB.auditLog) cleanDB.auditLog = [];
  cleanDB.auditLog.unshift({
    ts: new Date().toISOString(),
    user: currentUser?.username || 'admin',
    action: 'employee_deleted',
    details: `עובד נמחק: ${name}`
  });
  if (cleanDB.auditLog.length > 500) cleanDB.auditLog = cleanDB.auditLog.slice(0, 500);

  // שלב ד: כתוב ל-localStorage
  localStorage.setItem(DB_KEY, JSON.stringify(cleanDB));

  // שלב ה: כתוב ישירות ל-Firestore (בלי לעבור דרך getDB/pushToFirebase)
  if (firebaseConnected && firebaseDB) {
    try {
      await firebaseDB.collection('vacationSystem').doc('data').set({
        users:            JSON.stringify(cleanDB.users || {}),
        vacations:        JSON.stringify(cleanDB.vacations || {}),
        departments:      JSON.stringify(cleanDB.departments || []),
        approvalRequests: JSON.stringify(cleanDB.approvalRequests || []),
        deptManagers:     JSON.stringify(cleanDB.deptManagers || {}),
        auditLog:         JSON.stringify((cleanDB.auditLog || []).slice(0, 200)),
        settings:         JSON.stringify(cleanDB.settings || {}),
        permissions:      JSON.stringify(cleanDB.permissions || {}),
        announcements:    JSON.stringify(cleanDB.announcements || []),
        sick:             JSON.stringify(cleanDB.sick || {}),
        dailyStatus:      JSON.stringify(cleanDB.dailyStatus || {}),
        handovers:        JSON.stringify(cleanDB.handovers || {}),
        handoversArchive: JSON.stringify(cleanDB.handoversArchive || {}),
        handoverPending:  JSON.stringify(cleanDB.handoverPending || {}),
        orgTree:          JSON.stringify(cleanDB.orgTree || {}),
        timeClockRecords: JSON.stringify(cleanDB.timeClockRecords || {}),
        customQA:         JSON.stringify(cleanDB.customQA || []),
        updatedAt:        new Date().toISOString(),
        updatedBy:        currentUser?.username || 'admin'
      });
    } catch(e) {
      console.error('[DELETE] Firebase write failed:', e);
      showToast('⚠️ שגיאה בשמירה ל-Firebase', 'error');
    }
  }

  // שלב ו: הפעל listener מחדש אחרי 4 שניות
  setTimeout(() => {
    _isSyncing = false;
    if (firebaseDB) startRealtimeListener();
  }, 4000);
...
let _isSyncing = false;       // true while we're writing to Firebase — suppresses listener
let _lastPushTime = 0;        // timestamp of last successful push — listener ignores older snapshots
...
// Push local → cloud
async function pushToFirebase(dbOverride) {
  if (!firebaseConnected || !firebaseDB) return;
  _isSyncing = true;
  try {
    const db = dbOverride || getDB();
    console.log('[PUSH] users being sent to Firebase:', Object.keys(db.users || {}));
    await firebaseDB.collection('vacationSystem').doc('data').set({
    ...
    // Keep suppressing listener for 4s after write completes (Firebase snapshot delay)
    setTimeout(() => { _isSyncing = false; }, 4000);
  }
}
...
function startRealtimeListener() {
  ...
  _fbUnsubscribe = firebaseDB.collection('vacationSystem').doc('data')
    .onSnapshot(doc => {
      ...
      const data = doc.data();
      // Don't apply if we're currently writing or it was our own recent write
      if (_isSyncing) return;
      const updatedBy = data.updatedBy;
      const updatedAt = data.updatedAt ? new Date(data.updatedAt) : null;
      // Reject snapshot if it's older than our last push (stale data from Firebase)
      if (updatedAt && updatedAt.getTime() < _lastPushTime - 1000) return;
      const isOwnWrite = updatedBy === currentUser?.username
                      && updatedAt && (Date.now() - updatedAt.getTime()) < 5000;
...
// Override saveDB — saves locally; pushes to Firebase only when not already syncing
function saveDB(db) {
  _saveDBLocal(db);
  if (firebaseConnected && !_isSyncing) {
    pushToFirebase().catch(e => console.warn('saveDB push error:', e));
  }
}
