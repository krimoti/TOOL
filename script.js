...
async function deleteEmployee(username) {
  ...
  // עצור את ה-listener לפני הכל — confirm() חוסם JS ובזמנו ה-listener יכול לכתוב חזרה
  if (_fbUnsubscribe) { _fbUnsubscribe(); _fbUnsubscribe = null; }
  _isSyncing = true;
  _lastPushTime = Date.now();

  // 1. קרא מ-localStorage ישירות (לא דרך getDB שעלול לעשות ensureAdmin)
  const rawDB = JSON.parse(localStorage.getItem(DB_KEY) || '{}');
  console.log('[DELETE] לפני מחיקה — users:', Object.keys(rawDB.users || {}));
  delete rawDB.users[username];
  if (rawDB.vacations)        delete rawDB.vacations[username];
  if (rawDB.timeClockRecords) delete rawDB.timeClockRecords[username];
  if (rawDB.timeclock)        delete rawDB.timeclock[username];
  if (rawDB.sick) {
    Object.keys(rawDB.sick).forEach(k => { if (k.startsWith(username + '_')) delete rawDB.sick[k]; });
  }
  console.log('[DELETE] אחרי מחיקה מ-object — users:', Object.keys(rawDB.users || {}));

  // 2. כתוב ישירות ל-localStorage
  localStorage.setItem(DB_KEY, JSON.stringify(rawDB));
  console.log('[DELETE] אחרי כתיבה ל-localStorage — users:', Object.keys(JSON.parse(localStorage.getItem(DB_KEY)).users || {}));

  // 4. דחוף ל-Firebase ותחכה שהפעולה תסתיים
  try {
    await pushToFirebase(JSON.parse(localStorage.getItem(DB_KEY)));
    console.log('[DELETE] push הצליח — users ב-Firebase אמורים להיות:', Object.keys(JSON.parse(localStorage.getItem('vacSystem_v3') || '{}').users || {}));
  } catch(e) {
    console.error('[DELETE] push נכשל:', e);
  }

  // 6. תעד
  const dbAfter = JSON.parse(localStorage.getItem('vacSystem_v3') || '{}');
  if (!dbAfter.auditLog) dbAfter.auditLog = [];
  dbAfter.auditLog.unshift({ ts: new Date().toISOString(), user: currentUser?.username || 'admin', action: 'employee_deleted', details: `עובד נמחק: ${name}` });
  if (dbAfter.auditLog.length > 500) dbAfter.auditLog = dbAfter.auditLog.slice(0, 500);
  localStorage.setItem('vacSystem_v3', JSON.stringify(dbAfter));

  // 7. הפעל מחדש את ה-listener רק אחרי 3 שניות
  setTimeout(() => {
    _isSyncing = false;
