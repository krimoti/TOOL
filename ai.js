...
const DazuraAI = (() => {
  ...
  const CONV = [
    ...
    { test: t => /^(מה שלומ\S*|מה נשמע|מה מצב(ך|כם)?|מה קורה\S*|איך (אתה|את)\S*|how are you)/.test(t) && t.split(' ').length <= 5,
      reply: (t,u) => rand([
        `שלומי מצוין, תודה ששאלת **${fn(u)}**! 😊 מה אפשר לעשות בשבילך?`,
        `הכל טוב מצידי! מה שלומך, **${fn(u)}**? 🤖 יש משהו שאוכל לעזור בו?`,
...
יתרה: **${cb.balance.toFixed(1)} ימים**` : '');
  ...
  const CONV_GUARD = [
    /^(מה נשמע|מה שלומ|מה קורה|מה מצב)[?!\s]*$/,
    /^מי עובד[?!\s]*$/,
    /^מי נמצא[?!\s]*$/,
    /^(נכון|אכן|בדיוק|ודאי)[?!\s]*$/,
  ];

  // מילות domain — טקסט ללא אף אחת מהן לא יגיע ל-KB
  const DOMAIN_KEYWORDS = /חופש|יתרה|מחלה|wfh|עובד|מחלקה|מנהל|ניהול|נוכחות|שעות|firebase|סיסמה|בקש|אישור|תחזית|ניצול|מכסה|גיבוי|ייצוא|שכר|דוח|לוח|מצב|מי ב|מי ה|מי נ|כמה|מה ה|מה ז|איך|מתי|dazura|pwa|אפליק|צבע|גרס|ai/i;

  function isConvGuarded(raw) {
    const t = norm(raw);
    if (CONV_GUARD.some(rx => rx.test(t))) return true;
    // אם הטקסט ארוך יחסית אבל אין בו מילות domain → לא שייך ל-KB
    if (raw.length > 4 && !DOMAIN_KEYWORDS.test(raw)) return true;
    return false;
  }
...
• סטטוס היום: **${statusW}**`;
    ...
    // "מי בצוות/מחלקה שלי" — חייב לרוץ לפני vacation check
    if (/מי ב?צוות שלי|מי ב?מחלקה שלי|חברי הצוות|אנשי הצוות/.test(t)) {
      ...
      const todayTeam=dateKey(new Date());
      ctx.dept=myDept; ctx.resultList=members.map(u=>u.fullName);
      return `מחלקת **${myDept}** — ${members.length} עובדים:\n${members.map(u=>`• **${u.fullName}**${u.username===user.username?' (אתה)':''} ${TYPE_ICON[(db.vacations?.[u.username]||{})[todayTeam]]||'📍'}`).join('\n')}`;
    }

    // "כמה ימי חופש מגיעים לי" — QUOTA, לא vacation
    if (/כמה (ימי?|ימים|יום).*מגיע|מגיעים לי|כמה מגיע לי|מה המכסה/.test(t)) {
      const q = (user.quotas||{})[String(year)]||{};
      ctx.subject = 'quota';
      return `המכסה השנתית שלך לשנת ${year}: **${q.annual||0} ימים** (${((q.annual||0)/12).toFixed(1)} ימים לחודש).`;
    }

    if (/(?:^|\s)מי (ב(?!משרד|עבודה)|הוא|היא|יצא|בחופשה|חופש\b)|^מי לא מגיע|^מי נעדר/.test(t) && !/^כמה/.test(t) && !/במשרד|בעבודה|נמצא/.test(t)) {
    ...
    // ── כמה עובדים ───────────────────────────────────────────
    if (/כמה עובדים|מצבת|סה.?כ עובדים|כמה אנשים/.test(t)) {
      const users=Object.values(db.users||{}).filter(u=>u.status!=='pending'&&u.username!=='admin');
      const stats=statsForDate(db,dateKey(new Date()));
  ...
  function runKnowledge(raw, user, db) {
    ...
    // Guard: ביטויי שיחה מוגנים לא יגיעו ל-KB
    if (isConvGuarded(raw)) return null;

    // Priority match: WFH + חופשה → חייב להחזיר תשובה שמכילה WFH
    if (/wfh|מהבית|עבודה מהבית/.test(t) && /הבדל|לעומת|מול|חופשה/.test(t)) {
      return 'חופשה = יום חופש מנוכה מהיתרה. WFH = עבודה מהבית — לא נחשב חופשה, לא מנוכה מהיתרה. ✅';
    }

    // Priority match: ביטול חופשה — לפני הגשה
    if (/בטל|לבטל|ביטול/.test(t) && /חופש|בקשה/.test(t)) {
      return 'לפני אישור: לחץ על היום בלוח ← **בטל בקשה**. לאחר אישור: פנה למנהל לביטול ידני.';
    }

    // Priority match: "Firebase" ו-API key
    if (/firebase/.test(t) && /חבר|מחבר|הגדר|להגדיר|חיבור|איך/.test(t)) {
      return 'לשונית ניהול ← Firebase ← הכנס **apiKey** ו-projectId ← התחבר. ✅ **API Key** נמצא בהגדרות פרויקט Firebase.';
    }

    if (typeof AI_KNOWLEDGE !== 'undefined' && AI_KNOWLEDGE.length) {
      for (const entry of AI_KNOWLEDGE) {
        // בדיקת התאמה: שאלה ראשית + aliases — match מדויק בלבד (לא חלקי)
        const allQ = [entry.q, ...(entry.aliases || [])];
        const matched = allQ.some(q => {
          const qn = norm(q);
          // match מדויק: שוויון מוחלט, או includes עם אורך מינימלי 7 תווים
          // ומקסימום הפרש של 6 תווים (מונע false positives על ביטויים קצרים)
          if (t === qn) return true;
          if (qn.length >= 7 && t.includes(qn)) return true;
          if (t.length >= 7 && qn.includes(t) && Math.abs(t.length - qn.length) <= 6) return true;
          return false;
        });
        if (!matched) continue;
  ...
  function runFallback(raw, user, db) {
    ...
    // שאלות שיחתיות שלא זוהו ב-CONV → תגובה חכמה ולא "לא הבנתי"
    ...
    // ── טקסט ללא מילות מפתח מוכרות → תגובה ניטרלית ─────
    const hebrewWords = (raw.match(/[֐-׿]{3,}/g) || []).length;
    const knownKeywords = /חופש|יתרה|מחלה|wfh|עובד|מחלקה|מנהל|ניהול|נוכחות|שעות|firebase|סיסמה|בקשה|אישור|תחזית|ניצול|מכסה/i.test(raw);
    if (!knownKeywords && hebrewWords <= 2 && raw.length < 30) {
      return rand([
        `${fn(user)}, לא הבנתי בדיוק. 🤔 נסה לשאול על **יתרה**, **נוכחות** או **חופשה** — או כתוב **"עזרה"** לרשימה מלאה.`,
        `${fn(user)}, אפשר לפרט יותר? 😊 לדוגמה: "מה יתרת החופשה שלי?" / "מי בחופשה היום?"`,
      ]);
    }
