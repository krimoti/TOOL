...
` +
  ...
  function runFallback(raw, user, db) {
    ...
    // ── ניסיון אחרון: זיהוי שאלה קצרה + הצעת פרשנות ────────
    const shortQ = norm(rawInput).split(/\s+/).length <= 3;
    if (shortQ) {
      // שאלה קצרה — הצע כמה פרשנויות
  ...
  function classifyIntent(raw, user, db) {
    ...
    // שאלות יתרה קצרות — "כמה יש לי?" "מה יתרתי?"
    if (/^(כמה|מה) (יש לי|יתרתי|נשאר לי|נותר לי|היתרה|יתרת)/.test(t)) return 'balance';
    if (/^(כמה ימים|יתרה|יתרת חופשה)$/.test(t)) return 'balance';

    // שאלות "מי אני?" → פרופיל
    if (/^(מי אני|הפרופיל שלי|הפרטים שלי)$/.test(t)) return 'profile';

    // שאלות "איזו מחלקה" / "מה המחלקה שלי"
    if (/^(איזה? מחלקה|מה המחלקה שלי|במה מחלקה אני)/.test(t)) return 'dept_self';

    // שאלות "מה הסטטוס שלי היום?"
    if (/^(מה הסטטוס|סטטוס שלי|מה מצבי היום)$/.test(t)) return 'status_today';

    // "כמה ניצלתי?"
    if (/^(כמה ניצלתי|כמה לקחתי|ניצול)$/.test(t)) return 'used';

    // "מה תחזית?"
    if (/^(תחזית|מה תחזית)$/.test(t)) return 'forecast';

    // "מי בחופשה?" ללא תאריך
    if (/^(מי בחופשה|מי נעדר|מי לא מגיע)$/.test(t)) return 'who_vacation';

    // "מי WFH?"
    if (/^(מי wfh|מי מהבית|מי עובד מהבית)$/.test(t)) return 'who_wfh';

    // "מי חולה?"
    if (/^(מי חולה|מי מחלה)$/.test(t)) return 'who_sick';

    // "מה יש לי השבוע?" / "מה הבקשות שלי?"
    if (/^(מה יש לי|מה הבקשות שלי|בקשות שלי)$/.test(t)) return 'requests';

    return null; // לא זוהתה כוונה ספציפית
  }
  ...
  function respond(rawInput, currentUser, db) {
    ...
    if (intent === 'help')          r = respondHelp(currentUser, db);
      ...
      const members = Object.values(db.users||{}).filter(u => u.status!=='pending' && (Array.isArray(u.dept)?u.dept[0]:u.dept)===(Array.isArray(currentUser.dept)?currentUser.dept[0]:currentUser.dept));
      r = `אתה שייך למחלקת **${dept}** עם ${members.length} עובדים:\n${members.map(u=>`• ${u.fullName}${u.username===currentUser.username?' (אתה)':''}`).join('\n')}`;
    }
      ...
      r = runLiveData('מי אני', currentUser, db);
    }
    else if (intent === 'who_vacation') {
      const di = parseDate(rawInput);
      r = respondWhoAt(db, {date: new Date(), label:'היום', single:true}, currentUser, 'vacation');
    }
    else if (intent === 'who_wfh') {
      r = respondWhoAt(db, {date: new Date(), label:'היום', single:true}, currentUser, 'wfh');
    }
    else if (intent === 'who_sick') {
      r = respondWhoAt(db, {date: new Date(), label:'היום', single:true}, currentUser, 'sick');
    }
