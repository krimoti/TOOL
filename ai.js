...
const DazuraAI = (() => {
  ...
  // ── SYSTEM PROMPT — מגדיר התנהגות ורמת הבנה ──────────────
  // מבוסס על הנחיות המפתח (Chain of Thought, דיוק, תמציתיות)
  const SYSTEM_BEHAVIOR = {
    // זיהוי כוונה אמיתית — לפני כל תשובה
    analyzeIntent: (raw) => {
      const t = norm(raw);
      // שאלות שיחתיות — לא לשלוח ל-KB
      if (/^(מה נשמע|מה שלום|מה קורה|שלום|היי|תודה|להתראות|מזל טוב|כל הכבוד)[\s?!]*$/.test(t)) return 'SOCIAL';
      // שאלות נתונים חיים
      if (/יתרה|חופשה|מחלה|wfh|נוכחות|מחלקה|עובד/.test(t)) return 'DATA';
      // שאלות מערכת
      if (/ניהול|firebase|סיסמה|גיבוי|הגדרות|ייצוא/.test(t)) return 'SYSTEM';
      return 'GENERAL';
    }
  };
  ...
  const CONV = [
    ...
    { test: t => /^(מה שלומ|מה נשמע|מה מצב(ך|כם)?|מה קורה|איך אתה|איך את|how are you)[?!\s]*$/.test(t),
      reply: (t,u,db) => {
        const cb = calcBalance(u.username, new Date().getFullYear(), db);
        const balHint = cb && cb.balance > 0 ? ` אגב, יש לך **${cb.balance.toFixed(1)} ימי חופש** שמחכים לך 🏖️` : '';
        return rand([
          `שלומי מצוין, תודה ששאלת **${fn(u)}**! 😊 מה אפשר לעשות בשבילך?${balHint}`,
          `הכל טוב מצידי! מה שלומך, **${fn(u)}**? 🤖${balHint}`,
          `ממש טוב! **${fn(u)}**, יש משהו שאוכל לעזור בו? 😊${balHint}`,
        ]);
      }},
...
יתרה: **${cb.balance.toFixed(1)} ימים**` : '');
  ...
  // ── CONVERSATION GUARD: ביטויים שיחתיים שלא יופנו ל-KB ──
  const CONV_GUARD = [
    /^(מה נשמע|מה שלומ|מה קורה|מה מצב|איך אתה|איך את)/,
    /^(שלום|היי|הי|hey|hello|hi)[?!\s]*/,
    /^(בוקר טוב|בוקר אור|ערב טוב|לילה טוב)/,
    /^(תודה|תנקיו|thanks|thank you|יישר כח|כל הכבוד)/,
    /^(להתראות|ביי|bye|goodbye)/,
    /^מי עובד[?!\s]*$/,
    /^מי נמצא/,
    /^מי (במשרד|בעבודה|פיזי)/,
    /^(נכון|אכן|בדיוק|ודאי|סבבה|בסדר|אוקיי|אוקי|ממש)[?!\s.]*/,
    /^(נו+|ה+מ+|אה+|אממ)[?!\s]*/,
    /^מה (השעה|הזמן|התאריך|היום)/,
    /^(מי אתה|מה אתה)/,
    /^(מי יצר|מי בנה|מי פיתח)/,
  ];

  const CONV_STOP_WORDS = new Set([
    'שלום','היי','הי','תודה','ביי','נו','המ','אה','אוקיי','אוקי','סבבה','בסדר','ממש',
    'נכון','אכן','בדיוק','ודאי','ברור','כן','לא',
  ]);

  function isConvGuarded(raw) {
    const t = norm(raw);
    if (CONV_STOP_WORDS.has(t)) return true;
    return CONV_GUARD.some(rx => rx.test(t));
  }
...
` +
    ...
    if (/יתרה|יתרת|כמה (ימים|יום) (יש|נשאר|נותר|זמין)|כמה חופשה|מה היתרה|כמה נשאר לי/.test(t)) {
      // זיהוי שם עובד ספציפי בשאלה — "יתרת שרה", "מה יתרת דוד"
    ...
    // ── מי במשרד — חייב להיות לפני catch-all vacation ──────────
    if (/מי במשרד|מי (בחברה|בעבודה)|מי פיזי|מי מגיע|מי נמצא/.test(t)) {
      ctx.subject = 'office'; ctx.filter = 'office'; ctx.dateInfo = di;
      return respondWhoAt(db,di,user,'office');
    }

    // ── מי בצוות שלי — מחלקה עצמית ──────────────────────────
    if (/מי בצוות שלי|חברי הצוות|אנשי הצוות|מי במחלקה שלי|מחלקה שלי/.test(t)) {
    ...
    if (/מי (ב|הוא|היא|יצא|בחופשה|חופש)|מי לא מגיע|מי נעדר/.test(t)) {
    ...
    // "מי עובד" בלבד — תצוגת מצב צוות כללי (לא WFH ספציפי)
    if (/^מי עובד[\s?!]*$/.test(t)) {
      const stats=filterByDept(statsForDate(db,dateKey(di.date||new Date())),db,user);
      ctx.subject='vacation'; ctx.filter=null; ctx.dateInfo=di;
      ...
      return `**מצב הצוות — ${di.label||'היום'}:**\n🏖️ חופשה: ${stats.vacation.length} | 🏠 WFH: ${stats.wfh.length} | 🤒 מחלה: ${stats.sick.length} | 💼 במשרד: ${stats.office.length}` +
        ...
        `\n\nרוצה לדעת מי **WFH** / **חולה** / **בחופשה** בלבד?`;
    }

    // "מי עובד" לבדו → מצב צוות כללי
    if (/^מי עובד[\s?!]*$/.test(t)) {
      const statsTeam=filterByDept(statsForDate(db,dateKey(di.date||new Date())),db,user);
      ctx.subject='vacation'; ctx.filter=null; ctx.dateInfo=di;
      ctx.resultList=[...statsTeam.vacation,...statsTeam.wfh,...statsTeam.sick];
      return `**מצב הצוות — ${di.label||'היום'}:**\n🏖️ חופשה: ${statsTeam.vacation.length} | 🏠 WFH: ${statsTeam.wfh.length} | 🤒 מחלה: ${statsTeam.sick.length} | 💼 במשרד: ${statsTeam.office.length}`+
        (statsTeam.vacation.length?`\nבחופשה: ${statsTeam.vacation.join(', ')}`:'') +
        (statsTeam.wfh.length?`\nWFH: ${statsTeam.wfh.join(', ')}`:'') +
        (statsTeam.sick.length?`\nמחלה: ${statsTeam.sick.join(', ')}`:'') +
        `\n\nרוצה לסנן? שאל/י: "מי WFH?" / "מי חולה?" / "מי בחופשה?"`;
    }
  ...
  function getKBFuse() {
    ...
    _kbFuse = new BuiltinFuse(index, {
      ...
      threshold: 0.18,   // סף מחמיר — מונע false positives על nonsense
  ...
  function runKnowledge(raw, user, db) {
    ...
    // שלב 0: חסימה מוחלטת של שיחה חופשית — לא יגיעו ל-KB לעולם
    if (isConvGuarded(raw)) return null;

    // חסימה: שאלה קצרה מדי (≤2 מילים) שאינה שאלת מערכת ברורה
    const wordCount = t.split(/\s+/).length;
    const isSystemQ = /firebase|לשונית|ניהול|גיבוי|סיסמה|ייצוא|אדמין|מנהל|הגדרות|wfh|מחלה|חופשה|יתרה|מכסה|דוח|אקסל|pwа|נייד/.test(t);
    if (wordCount <= 2 && !isSystemQ) return null;
    ...
    if (typeof AI_KNOWLEDGE !== 'undefined' && AI_KNOWLEDGE.length) {
      for (const entry of AI_KNOWLEDGE) {
        ...
        const matched = allQ.some(q => {
          const qn = norm(q);
          return t === qn
            || (qn.length >= 6 && t.includes(qn))
            || (t.length >= 6 && qn.includes(t) && Math.abs(t.length - qn.length) <= 8);
        });
        if (!matched) continue;
    ...
    // שלב 2: Fuse לתיקון שגיאות כתיב (סף מחמיר: 0.18)
  ...
  function runFallback(raw, user, db) {
    ...
    // ── חסימה: שיחה חופשית שהגיעה לכאן בטעות ──────────────
    if (isConvGuarded(raw)) {
      return rand([
        `${fn(user)}, שמח שאתה/את כאן! 😊 מה אוכל לעזור לך היום?`,
        `הי **${fn(user)}**! איך אפשר לעזור? 🤖`,
        `**${fn(user)}** — יש משהו שאוכל לעשות בשבילך? 😊`,
      ]);
    }

    // ── זיהוי כוונה לפי SYSTEM_BEHAVIOR ─────────────────────
    const intentType = SYSTEM_BEHAVIOR.analyzeIntent(raw);

    // שאלות שיחתיות שלא זוהו ב-CONV
    if (intentType === 'SOCIAL') {
      return rand([
        `${fn(user)}, שמח שאתה/את כאן! 😊 מה אוכל לעזור לך היום?`,
        `הי **${fn(user)}**! איך אפשר לעזור? 🤖`,
        `**${fn(user)}** — יש משהו שאוכל לעשות בשבילך? 😊`,
  ...
  function classifyIntent(raw, user, db) {
    ...
    // ── חסימת שיחה חופשית — לא יגיעו ל-KB ─────────────────
    if (isConvGuarded(raw)) return '__conv__';

    // ── יתרה ───────────────────────────────────────────────
    // אם יש שם עובד בשאלה — לא לתפוס כ-balance, ל-runLiveData יטפל בזה
    if (/^(כמה|מה) (יש לי|יתרתי|נשאר לי|נותר לי|היתרה|יתרת)/.test(t) && !t.includes(' של ') && !t.includes(' עבור ') && t.split(' ').length <= 6) {
      // בדוק אם השאלה מתייחסת לעצמי בלבד (ללא שם עובד)
      const hasEmpName = /יתרת\s+\S+/.test(t) && !/יתרת\s*(ה?חופש|שנה|ה?שנתית)/.test(t);
      if (!hasEmpName) return 'balance';
    }
    if (/^(כמה ימים|יתרה|יתרת חופשה|כמה ימי חופש)$/.test(t)) return 'balance';

    // ── מכסה ───────────────────────────────────────────────
    if (/^(מה המכסה|כמה ימים מגיע|המכסה שלי|מכסה שנתית|כמה ימים בשנה)/.test(t)) return 'quota';
    ...
    if (/^(מי במשרד|מי בעבודה|מי מגיע|מי נמצא)[?!\s]*$/.test(t)) return 'who_office';
  ...
  function respond(rawInput, currentUser, db) {
    ...
    else if (intent === '__conv__') {
      // שיחה חופשית מוגנת — נסה CONV קודם, אחר כך fallback חכם
      r = runConversation(rawInput, currentUser, db);
      if (!r) {
        // תגובה גנרית חכמה לשיחה שלא נמצאה ב-CONV
        const cb = calcBalance(currentUser.username, year, db);
        const balHint = cb ? ` יש לך **${cb.balance.toFixed(1)} ימי חופש** 🏖️` : '';
        r = rand([
          `שלומי מצוין, **${fn(currentUser)}**! 😊 מה אפשר לעשות בשבילך?${balHint}`,
          `הכל טוב! **${fn(currentUser)}**, יש משהו שאוכל לעזור בו? 🤖${balHint}`,
        ]);
      }
    }
    else if (intent === 'quota') {
      const q = (currentUser.quotas||{})[String(year)]||{};
      r = `המכסה השנתית שלך לשנת ${year}: **${q.annual||0} ימים** (${((q.annual||0)/12).toFixed(1)} ימים לחודש).`;
      ctx.subject = 'quota';
    }
